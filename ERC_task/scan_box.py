...
def main():
    ...
    _, x, y, z, qx, qy, qz, qw = [13,0.4130681132329064,-0.24107263940844223,0.20694506990134637,0.0004569625622637568,-0.00018015395191389738,-0.809051042083999,0.5877381815337213]

    current_pose = move_group.get_current_pose().pose
    pose_goal = Pose()
    pose_goal = current_pose
    pose_goal.position.x = x - 0.1
    pose_goal.position.y = y + 0.02
    pose_goal.position.z = z + 0.1

    # Convert Euler angles (roll, pitch, yaw) to quaternion
    roll = 0
    pitch = 1.5708 * 2  # 90 degrees in radians (looking down)
    yaw = 0
